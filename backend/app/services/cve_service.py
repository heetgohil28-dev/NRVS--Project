...
from app.database.models import Vulnerability, HostResult, PortResult, SeverityLevel
...
def _parse_cvss(cve_item: Dict[str, Any]) -> tuple[Optional[float], Optional[float], Optional[str]]:
...
def _extract_cve_ids_from_port(port: PortResult) -> List[str]:
    cve_ids = []
...
async def enrich_host_vulnerabilities(
    ...
    if not all_cve_ids:
        logger.info("No CVEs found for host %s", host.ip_address)
        return []
