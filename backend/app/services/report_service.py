...
from app.database.models import ScanJob, HostResult, Vulnerability, Report, ScanStatus
...
jinja_env.loader = FileSystemLoader(TEMPLATES_DIR)
...
def _get_scan_context(scan_id: int, db: Session) -> dict:
    ...
    for host in hosts:
        ...
        critical_count += sum(1 for v in vulns if str(v.severity) in ("critical", "SeverityLevel.CRITICAL"))
        high_count     += sum(1 for v in vulns if str(v.severity) in ("high",     "SeverityLevel.HIGH"))
        medium_count   += sum(1 for v in vulns if str(v.severity) in ("medium",   "SeverityLevel.MEDIUM"))
        low_count      += sum(1 for v in vulns if str(v.severity) in ("low",      "SeverityLevel.LOW"))
